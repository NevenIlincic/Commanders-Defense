...
use crate::{lobby::LobbyHandler, network_protocol::{CreateLobbyRequest, JoinRequest, LobbiesInfo, ServerMessage}};
...
impl RestService{
    ...
    pub async fn get_lobbies_list(State(state): State<Arc<Mutex<LobbyHandler>>>) -> impl IntoResponse {
        let lobby_handler = state.lock().await;
        let response_bytes = match bincode::serialize(&ServerMessage::LobbiesList(LobbiesInfo::new(&lobby_handler))) {
            Ok(bytes) => bytes,
            Err(_) => return StatusCode::INTERNAL_SERVER_ERROR.into_response(),
        };
